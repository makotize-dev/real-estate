...
//   ※ 8/21 の保守枠では「LEVEL2 は実際には一度も発火していない」と書いたが、
//     **2026-09-11 第2セッションに全76 transcript を走査したところ誤りだった**＝
//     LEVEL2 は 10 セッションで発火していた（538msgs/709千字・696/704・711/707・
//     795/773・861/728・902/504・910/566・932/596・938/711・949/746）。LEVEL1 は 38 回。
//     **字数側で先に当たる回と、メッセージ数側で先に当たる回の両方がある**（2つのOR条件は
//     どちらも実際に効いている）。
//   ※ 2026-09-11 第3セッション（保守枠）に CLAUDE.md 行動トリガー表の見出しを
//     「スレッド長の警告が出た（LEVEL1・LEVEL2 のどちらでも）」へ変更し、LEVEL2 が
//     表から漏れていた状態を解消した。**対応は2レベルとも同じ**（上記のとおり文面が
//     既に統一されているため）。あわせて、先に LEVEL2 に当たると writeGuard で l1 も
//     同時に立つので **LEVEL1 が一度も出ないまま LEVEL2 が初出しうる**ことを表に明記した。
//
// 【他フックとの噛み合わせ・2026-09-11 第2セッション】
//   このフックのブロックは transcript に `type:"user" / isMeta:true` の
//   「Stop hook feedback:」行として残る。**その差し戻しで書き直した内容を
//   `no-fabricated-turn.js` が「2回目の採点」と数えて連鎖ブロックする事故**が
//   2026-09-06 に起きた（4回連続でブロックし上限を使い切った）。**対策は
//   no-fabricated-turn.js 側**＝差し戻し行を見つけたらそれ以前の下書きを数えない。
//   → このファイルの文面や閾値を変えるときは、向こうの検出も一緒に確認する
//      （回帰テスト＝`.claude/hooks/no-fabricated-turn.test.js`）。
