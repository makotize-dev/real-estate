...
- **週次進捗テーブル**：当該週の「実績」「過不足」「累積」「主な学習トピック」を記入する
  - 実績時間はセッション開始時にユーザーに確認する（「今日は何時間勉強しましたか？」）
  - 過不足 = 実績 − 計画（9h）、累積 = 前週までの累積 + 今週の過不足
  - 週が変わった場合は次の週の行に移る（前の週は空白のまま確定する）
- **科目別カバレッジ**：「学習済み条文・トピック」と「残り・次の予定」を更新する
- **セッション一覧**：新しいセッションを**先頭**に追加（実績時間も記入）
